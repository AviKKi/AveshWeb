...
const CodeEntryCard = withStyles(styles)((props)=>{
    ...
    return (
        <Card  className={classes.card}>
            ...
            <CardActions>
                <Button onClick={couponClick} color="secondary" variant="outlined">
                    Redemee
                </Button>
            </CardActions>
        </Card>
...
class PaymentOptions extends React.Component  {
    ...
    return (
        <div style={{overflow:"hidden"}}>
            ...
            {(spinner || accomodationflag===true)?<></>:
                (<Grid  justify="center" container spacing={8}>
                <Grid xs={8} lg={4} item>
                    <Option
                        clickCallback={async()=>await makePayment("accomf", token, dispatch)}
                ...
                <Grid xs={8} lg={4} item>
                    <Option clickCallback={async()=>await makePayment("accomm", token, dispatch)} title="Accomodation for Boys(+Free T-shirt)" desciption="Get accomodation at just Rs. 270(+150 Caution Money) with a free T-shirt." />
                </Grid >
            </Grid>)}
        </div>
    )
